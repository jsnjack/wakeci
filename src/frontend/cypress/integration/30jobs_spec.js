describe('Jobs page', function () {
    it('should create a new job', function () {
        ...
        cy.get(`tr[data-cy=${jobName}]`).scrollIntoView().should('be.visible');
    ...
    it('should edit a job', function () {
        ...
        for (let index = 0; index < 5; index++) {
            cy.get('[data-cy=editor] .CodeMirror-code')
                .type('{selectall}')
                .type('{selectall}')
                .type('{backspace}');
        }
    ...
    it('should filter jobs', function () {
        ...
        cy.get('[data-cy=filter-jobs]').clear().type(jobName);
        cy.get('tr').should('be.visible').should('have.length', 1);
        cy.get(`tr[data-cy=${jobName}]`).scrollIntoView().should('be.visible');
    });
});
